...
function static_target(root, options, callback) {
    ...
    try {
        ...
        // Build map for a GenericMessage
        config.genericMessageMap = root.GenericMessage.fieldsArray.reduce((result, item) => {
...
function buildType(ref, type) {
    ...
    pushComment([
        ...
        "@constructor",
        "@param {Partial<" + exportName(type, true) + ">} [" + (config.beautify ? "properties" : "p") + "] Properties to set"
    ]);
...
*/
         ...
         //   push(escapeName(type.name) + ".prototype" + prop + ";");
        }
    });
    ...
    if (config.decode) {
        ...
        // For now need to avoid a mess with required/optional fields
    ...
    const genericMessageField = config.genericMessageMap[type.name];
    if (genericMessageField){
        push("");
        pushComment([
            "Converts " + aOrAn(type.name) + " message to GenericMessage",
            "@function toGenericMessage",
            "@memberof " + exportName(type),
            "@instance",
            "@returns {GenericMessage} GenericMessage"
        ]);
        push(escapeName(type.name) + ".prototype.toGenericMessage = function toGenericMessage() {");
        ++indent;
            push(`return new $root.GenericMessage({MessageId: ${genericMessageField.id}, ${genericMessageField.name} : this});`);
        --indent;
        push("};");
    }
