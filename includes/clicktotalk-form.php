...
class WP3CXC2C_ClickToTalkForm {
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'Click To Talk items', '3cx-clicktotalk' ),
				'singular_name' => __( 'Click To Talk item', '3cx-clicktotalk' ),
			),
			'rewrite' => false,
			'query_var' => false,
		) );
	...
	public function form_html( $args = '' ) {
		...
		$p = explode('/callus#',$pbxurl.'/callus#');
		$phonesystemurl = $p[0];
		$party = $p[1];
		$enablephone = ($config['aspect'] == 'both' || $config['aspect'] == 'phone') ? 'true' : 'false';
		$minimized = ($config['minimized']) ? 'true' : 'false';
		...
		$html='<'.$tag.' id="'.$this->id.'" style="position: fixed; bottom: 8px; right: 8px;"
         phonesystem-url="'.$phonesystemurl.'"
         party="'.$party.'"';
		if ($tag=='call-us'){
				 $html.='invite-message="'.$config['welcomemessage'].'" 
		 allow-call="'.$enablephone.'"
		 operator-name="'.$config['welcomemessagesender'].'" 
				 allow-video="'.$enablevideo.'"
				 minimized="'.$minimized.'"
				 window-title="'.$config['chatboxtitle'].'" 
