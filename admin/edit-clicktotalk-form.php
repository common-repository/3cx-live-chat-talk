...
function wp3cxc2c_admin_save_button( $post_id ) {
	...
	$button = sprintf(
		'<input type="submit" class="button-primary" name="wp3cxc2c-save" value="%1$s" onclick="%2$s" />',
...
<h1 class="wp-heading-inline"><?php
	if ( $post->initial() ) {
		echo esc_html( __( 'Add New Click To Talk item', '3cx-clicktotalk' ) );
	} else {
		echo esc_html( __( 'Edit Click To Talk item', '3cx-clicktotalk' ) );
	}
?></h1>
...
<div id="delete-action">
	<input type="submit" name="wp3cxc2c-delete" class="delete submitdelete" value="<?php echo esc_attr( __( 'Delete', '3cx-clicktotalk' ) ); ?>" <?php echo "onclick=\"if (confirm('" . esc_js( __( "You are about to delete this Click To Talk item.\n  'Cancel' to stop, 'OK' to delete.", '3cx-clicktotalk' ) ) . "')) {this.form._wpnonce.value = '$delete_nonce'; this.form.action.value = 'delete'; return true;} return false;\""; ?> />
</div><!-- #delete-action -->
<?php endif; ?>
...
<div class="inside">
	<p><?php echo esc_html( __( "Check 3CX Webmeeting resources for further details.", '3cx-clicktotalk' ) ); ?></p>
	<ol>
		<li><?php echo sprintf(
			...
			wp3cxc2c_link(
				__( 'https://www.3cx.com/phone-system/web-conferencing/', '3cx-clicktotalk' ),
